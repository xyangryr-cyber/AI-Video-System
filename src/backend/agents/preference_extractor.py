...
class PreferenceExtractor:
    ...
    @staticmethod
    def _parse(utterance: str) -> tuple[str, float | str] | None:
        ...
        if isinstance(raw, _CandidateList):
            ...
            for c in raw.candidates:
                if c.confidence >= CONFIDENCE_MIN:
                    action: Literal["auto_save", "ask_writeback"] = (
                        "auto_save" if c.confidence > AUTO_SAVE_THRESHOLD else "ask_writeback"
                    )
                    results.append(
                        ExtractedPreference(
                            id=c.id,
    ...
    def compare_for_writeback(
        self,
        *,
        actual_audio_settings: dict[str, Any] | None = None,
    ...
    def extract_for_confirm(
        ...
        **_kwargs: Any,
        ...
        return {
            ...
            "confidence": result.confidence,
            "required_action": ("skip" if result.proposed_action == "auto_save" else "confirm"),
