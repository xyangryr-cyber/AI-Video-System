...
export function CreateProjectForm({ onSubmit, isSubmitting, onCancel }: Props): ReactElement {
  ...
  return (
    <form
      ...
      className="bg-white rounded-2xl border-2 border-slate-200 p-8 shadow-sm space-y-6 max-w-xl"
    >
      <div>
        ...
        />
        {titleTouched && !title.trim() && <p className="text-red-500 text-xs mt-1">标题不能为空</p>}
      </div>
      <div>
        ...
        <div className="flex justify-between items-center mt-1">
          <span
            className={`text-xs ${description.length > 0 && description.length < MIN_DESC ? "text-red-500" : "text-slate-400"}`}
          >
            至少 10 个字符
          </span>
