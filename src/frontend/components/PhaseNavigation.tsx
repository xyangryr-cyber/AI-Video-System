  ...
  const allPhases =
    phases.length >= 13
      ...
      : [
          ...phases,
          ...Array.from(
            { length: Math.max(0, 13 - phases.length) },
            (_, i) => ({ phase: phases.length + i }) as PhaseItem,
          ),
        ];
      ...
      <div className="flex-1 w-full flex flex-col items-center gap-4 overflow-y-auto py-2">
        {allPhases.map((p) => {
          ...
          return (
            ...
            >
              <span
                className={`text-[11px] font-black ${isActive ? "opacity-100" : "opacity-60 group-hover:opacity-100"}`}
              >
                P{p.phase}
              </span>
