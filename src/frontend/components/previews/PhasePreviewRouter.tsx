...
export function PhasePreviewRouter({ phase, artifactData }: PhasePreviewRouterProps) {
  switch (phase) {
    case 0:
      return <P0RequirementsView requirements={artifactData as RequirementsJSON | undefined} />;
    case 1: {
      const versions = artifactData?.["versions"] as Array<Record<string, unknown>> | undefined;
      const content = buildP1Content(versions);
      return <P1ScriptView content={content || undefined} versions={versions?.length} />;
    }
    case 2:
      return (
        <P2SegmentView
          segments={
            (Array.isArray(artifactData) ? artifactData : artifactData?.["segments"]) as
              | ScriptSegment[]
              | undefined
          }
        />
    ...
    case 3: {
      ...
      const content =
        segments
          ...
          .filter(Boolean)
          .join("\n\n") || "";
      const voice_directions = segments?.map((s) => s["voice_direction"]).filter(Boolean) as
        | VoiceDirection[]
        | undefined;
      return (
        <P3VoiceScriptView content={content || undefined} voice_directions={voice_directions} />
      );
    }
    case 4:
      return (
        <P4SegmentAudioPlayer segments={artifactData?.["segments"] as AudioSegment[] | undefined} />
      );
    case 5:
      return (
        <P5WaveformPlayer
    ...
    case 6:
      return <P6SfxListPlayer sfx_list={artifactData?.["sfx_list"] as SfxItem[] | undefined} />;
    case 7:
      return (
        <P7StoryboardGallery shots={artifactData?.["shots"] as StoryboardShot[] | undefined} />
      );
    case 8:
      return <P8FrameGallery frames={artifactData?.["frames"] as VideoFrame[] | undefined} />;
    case 9:
      return <P9BrollGallery brolls={artifactData?.["brolls"] as BrollItem[] | undefined} />;
    case 10:
      return (
        <P10VideoPlayer
