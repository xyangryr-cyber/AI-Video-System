...
export function P7StoryboardGallery({ shots }: P7StoryboardGalleryProps) {
  if (!shots || shots.length === 0) {
    return (
      <div data-testid="preview-p7" className="flex flex-col items-center justify-center h-64 text-slate-400">
  ...
  return (
    <div data-testid="preview-p7" className="relative border-l-2 border-slate-200 ml-4 pl-8 space-y-6 pb-4 pt-2">
      {shots.map((shot) => {
        // Normalize API data to component fields (API uses snake_case, may differ from TS type)
        const shotId = (shot as any).shot_id || shot.id || '?';
        const duration = shot.duration_sec
          || ((shot as any).time_range?.end_seconds - (shot as any).time_range?.start_seconds)
          || 0;
        const description = shot.description || (shot as any).narration_text || (shot as any).content || '';
        const templateId = shot.template_id || (shot as any).template_type;
        const shotType = (shot as any).type || 'template';
            ...
            <div className="bg-white border-2 border-slate-200 rounded-2xl p-5 shadow-sm space-y-4 hover:border-blue-300 transition-colors">
              ...
              <div className="flex flex-wrap md:flex-nowrap justify-between gap-3 border-b border-slate-100 pb-3">
                <div className="flex items-center gap-3">
                  <span className="bg-[#1e293b] text-white font-bold px-2 py-0.5 rounded text-xs tracking-widest">{shotId}</span>
                  <span className="font-mono text-slate-500 text-[13px] font-bold">{timeRange}</span>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-[13px] text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100 italic leading-relaxed">"{description}"</p>
                  </div>
                  <div>
                    <h4 className="flex items-center text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-2">
                ...
                <div className="space-y-4 md:border-l md:border-slate-100 md:pl-5">
                  <div>
                    ...
                    <p className="text-[13px] font-medium text-slate-600">模版: {templateId || shotType} / 时长: {duration}s</p>
                  </div>
                  <div>
                    <h4 className="flex items-center text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-2">
