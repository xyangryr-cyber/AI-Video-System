...
async function createProjectAndWait(
  ...
  title: string,
  desc: string,
): Promise<{ projectId: string; navigated: boolean }> {
  ...
  try {
    await page.waitForURL((url: URL) => url.pathname.match(/\/projects\/proj_/) !== null, {
      timeout: 15000,
    });
...
test.describe("场景 1: 创建项目 + 生成完整需求定义", () => {
  ...
  test("步骤 1.1 — 进入创建项目页面", async ({ page }) => {
    ...
    record(
      SCENARIO,
      "1.1",
      "1.1b 点击后地址栏跳转到 /projects/new",
      page.url().includes("/projects/new") ? "PASS" : "FAIL",
      `URL: ${page.url()}`,
    );
    ...
    if (hasTitle && hasDesc && hasSubmit) {
      ...
      record(
        SCENARIO,
        "1.1",
        "1.1c 标题输入框、描述输入框、提交按钮存在",
        "FAIL",
        `title=${hasTitle} desc=${hasDesc} submit=${hasSubmit}`,
      );
    }

    await page.screenshot({
      path: "test-results/phase0-s1-step1.1-create-form.png",
      fullPage: true,
    });
  });
  ...
  test("步骤 1.2 — 输入标题和完整描述并提交", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/phase0-s1-step1.2-project-page.png",
      fullPage: true,
    });
    ...
    record(
      ...
      phaseNavVisible
        ? "PhaseNavigation sidebar 渲染"
        : "PhaseNavigation sidebar 不存在于 WorkflowPage 中",
    );
    ...
    // Loading indicator
    const hasLoading =
      pageText.includes("执行中") || pageText.includes("生成") || pageText.includes("加载");
    record(
      SCENARIO,
      "1.2",
      "1.2f 加载/处理中指示器",
  ...
  test("步骤 1.3 — 等待 RequirementsAgent 生成完成", async ({ page }) => {
    ...
    const pageText = await page.locator("body").innerText();
    await page.screenshot({
      path: "test-results/phase0-s1-step1.3-after-agent.png",
      fullPage: true,
    });

    const hasStructured =
      pageText.includes("需求") || pageText.includes("时长") || pageText.includes("平台");
    record(
      SCENARIO,
      "1.3",
      "1.3a 结构化需求摘要显示",
    ...
    const hasPlatformSpec =
      pageText.includes("1920") || pageText.includes("1080") || pageText.includes("bilibili");
    record(
      SCENARIO,
      "1.3",
      "1.3b B站平台规格显示",
    ...
    const hasWordCount =
      pageText.includes("字数") || pageText.includes("1536") || pageText.includes("2880");
    record(
      SCENARIO,
      "1.3",
      "1.3c 目标字数范围",
  ...
  test("步骤 1.4 — CompletenessReviewer 审核通过 + 推进按钮", async ({ page }) => {
    ...
    const pageText = await page.locator("body").innerText();
    await page.screenshot({
      path: "test-results/phase0-s1-step1.4-review-check.png",
      fullPage: true,
    });
...
test.describe("场景 2: 自然对话修改需求", () => {
  ...
  test("步骤 2.1 — revise 修改特定字段 (时长)", async ({ page }) => {
    ...
    const pageText = await page.locator("body").innerText();
    await page.screenshot({
      path: "test-results/phase0-s2-step2.1-after-revise.png",
      fullPage: true,
    });
    ...
    record(
      SCENARIO,
      "2.1",
      "2.1d 任务清单出现 user_revision",
      hasReviseTask ? "PARTIAL" : "FAIL",
      "mock 任务数据不反映真实 Agent 任务",
    );

    const hasDurationChange =
      pageText.includes("15") && (pageText.includes("分钟") || pageText.includes("min"));
    record(
      SCENARIO,
      "2.1",
      "2.1e 需求摘要更新",
      hasDurationChange ? "PARTIAL" : "FAIL",
      "mock 数据不反映 Agent 输出",
    );
  });
  ...
  test("步骤 2.2 — revise 修改发布平台", async ({ page }) => {
    ...
    record(
      ...
      "2.2",
      "2.2b 视频号竖屏规格",
      hasVertical ? "PASS" : "FAIL",
      hasVertical ? "检测到竖屏规格" : "无视频号规格",
    );
  ...
  test("步骤 2.3 — regenerate 重新生成", async ({ page }) => {
    ...
    const pageText = await page.locator("body").innerText();
    record(SCENARIO, "2.3", "2.3a regenerate 请求", "PARTIAL", "mock 数据无法反映 Router 意图识别");
    await page.screenshot({
      path: "test-results/phase0-s2-step2.3-regenerate.png",
      fullPage: true,
    });
  });
  ...
  test("步骤 2.4 — Router 模糊意图 → clarify", async ({ page }) => {
    ...
    const pageText = await page.locator("body").innerText();
    const hasClarify =
      pageText.includes("澄清") || pageText.includes("不太确定") || pageText.includes("你想");
    record(
      SCENARIO,
      "2.4",
      "2.4a Agent 澄清回复",
    ...
    record(SCENARIO, "2.4", "2.4b clarify 不改动任务账本", "PARTIAL", "mock 数据无法验证");
...
test.describe("场景 3: 澄清问题处理", () => {
  ...
  test("步骤 3.1 — 用不完整描述创建项目", async ({ page }) => {
    ...
    const pageText = await page.locator("body").innerText();
    await page.screenshot({
      path: "test-results/phase0-s3-step3.1-incomplete-desc.png",
      fullPage: true,
    });

    const hasClarification =
      pageText.includes("需要补充") || pageText.includes("澄清") || pageText.includes("⚠");
    record(
      SCENARIO,
      "3.1",
      "3.1a 澄清问题区域",
      hasClarification ? "PASS" : "FAIL",
      "无澄清区域。WorkflowPage mock 数据不包含 clarification_needed",
    );
    ...
    record(
      ...
      "3.1",
      "3.1b 澄清维度覆盖",
      coverage >= 1 ? "PARTIAL" : "FAIL",
      `覆盖 ${coverage}/3 维度`,
    );

    const noFabrication = !pageText.includes("万亿");
    record(SCENARIO, "3.1", "3.1c 无编造细节", noFabrication ? "PASS" : "FAIL", "无编造数据");
  ...
  test("步骤 3.2 — 通过对话补充缺失信息", async ({ page }) => {
    ...
    record(SCENARIO, "3.2", "3.2a 需求更新", hasUpdate ? "PARTIAL" : "FAIL", "mock 数据不反映更新");

    record(SCENARIO, "3.2", "3.2b 澄清区域缩小", "PARTIAL", "mock 数据无动态变化");
    record(SCENARIO, "3.2", "3.2c 字段标记变化", "PARTIAL", "mock 数据无字段标记");
    await page.screenshot({
      path: "test-results/phase0-s3-step3.2-supplement.png",
      fullPage: true,
    });
  });
  ...
  test("步骤 3.3 — 补充后审核通过", async ({ page }) => {
    ...
    record(
      ...
      "3.3",
      "3.3a review 通过",
      hasPassed ? "PARTIAL" : "FAIL",
      "mock 数据不显示 review 结果",
    );
    ...
    await page.screenshot({
      path: "test-results/phase0-s3-step3.3-review-passed.png",
      fullPage: true,
    });
  });
});
...
test.describe("场景 4: 偏好提取与确认", () => {
  ...
  test("步骤 4.1~4.4 — 偏好提取->检查->确认->门禁通过", async ({ page }) => {
    ...
    if (btnVisible) {
      ...
      await page.screenshot({
        path: "test-results/phase0-s4-preference-modal.png",
        fullPage: true,
      });
    } else {
...
test.describe("场景 6: 输入校验与边界条件", () => {
  ...
  test("步骤 6.1 — 描述不足 10 字时阻止提交", async ({ page }) => {
    ...
    record(
      SCENARIO,
      "6.1",
      "6.1a 页面停留在 /projects/new",
      stillOnNew ? "PASS" : "FAIL",
      `停留在页=${stillOnNew}`,
    );
    ...
    await page.screenshot({
      path: "test-results/phase0-s6-step6.1-validation.png",
      fullPage: true,
    });
  });
  ...
  test("步骤 6.3 — 页面恢复测试", async ({ page }) => {
    ...
    record(SCENARIO, "6.3", "6.3 页面恢复成功", pageLoaded ? "PASS" : "FAIL", `加载=${pageLoaded}`);
  });
});
...
test.describe("综合: UI 交互细节验证", () => {
  ...
  test("项目列表页功能检查", async ({ page }) => {
    ...
    record(
      ...
      "7.1",
      "7.1a 项目列表表格",
      tableVisible ? "PASS" : "FAIL",
      `table=${tableVisible}`,
    );

    const rows = page.locator("table tbody tr");
    const rowCount = await rows.count();
    record(SCENARIO, "7.1", "7.1b 项目行数", rowCount > 0 ? "PASS" : "FAIL", `行数=${rowCount}`);
  ...
  test("WorkflowPage 完整 UI 检查", async ({ page }) => {
    ...
    record(
      ...
      "7.2",
      "7.2c 产物列表",
      pageText.includes("已生成产物") ? "PASS" : "FAIL",
      "ArtifactList 区域",
    );
    ...
    record(
      ...
      "7.2",
      "7.2d 事实核查面板",
      pageText.includes("事实核查") ? "PASS" : "FAIL",
      "FactualLedger 区域",
    );
    ...
    if (await textarea.isVisible().catch(() => false)) {
      ...
      const msgAppeared =
        afterText.includes("测试发送消息到 Agent") && afterText.length > beforeText.length;
      record(
        SCENARIO,
        "7.2",
        "7.2e 消息发送后出现在对话区",
        ...
        msgAppeared
          ? "消息成功显示"
          : "消息未出现在对话区 — handleSend 是空函数 (no-op)。对话修改功能完全不可用",
      );
    }
...
// ═══════════════════════════════════════════════════════════════
// 场景 8: 任务清单行为验证 (FR-020, FR-021) — T035
// ═══════════════════════════════════════════════════════════════
test.describe("场景 8: 任务清单行为验证 (FR-020, FR-021)", () => {
  const SCENARIO = "场景8-任务清单";

  test("步骤 8.1 — 默认折叠视图仅显示当前执行中任务", async ({ page }) => {
    const { navigated } = await createProjectAndWait(
      page,
      "任务清单折叠测试",
      "分析黄金走势，时长约5分钟，发在B站，面向新手投资者",
    ...
    if (!navigated) {
      record(SCENARIO, "8.1", "8.1 项目创建", "FAIL", "无法创建项目");
      return;
    }
    ...
    record(
      SCENARIO,
      "8.1",
      "8.1a 任务清单卡片存在 (FR-020)",
      taskCardVisible ? "PASS" : "FAIL",
      `visible=${taskCardVisible}`,
    );

    const pageText = await page.locator("body").innerText();
    const hasCurrentTask =
      pageText.includes("执行中") || pageText.includes("running") || pageText.includes("生成");

    record(
      SCENARIO,
      "8.1",
      "8.1b 默认折叠视图仅显示当前任务 (FR-020)",
      hasCurrentTask ? "PASS" : "FAIL",
      `当前任务指示=${hasCurrentTask}`,
    );

    await page.screenshot({
      path: "test-results/phase0-s8-step8.1-collapsed-tasks.png",
      fullPage: true,
    });
  });

  test("步骤 8.2 — 展开按钮显示所有任务", async ({ page }) => {
    const { navigated } = await createProjectAndWait(
      page,
      "任务清单展开测试",
      "分析黄金走势，时长约5分钟，发在B站，面向新手投资者",
    ...
    if (!navigated) {
      record(SCENARIO, "8.2", "8.2 项目创建", "FAIL", "无法创建项目");
      return;
    }
    ...
    const expandBtn = page
      .locator("button")
      .filter({ hasText: /展开|显示全部|expand|更多/ })
      .first();
    ...
    if (expandBtnVisible) {
      ...
      const taskItems = page.locator('[data-testid="task-card"] [data-testid^="task-item"]');
      const expandedCount = await taskItems.count().catch(() => 0);
      record(
        SCENARIO,
        "8.2",
        "8.2a 展开后显示所有任务 (FR-020)",
        expandedCount > 1 ? "PASS" : "FAIL",
        `展开后可见任务数=${expandedCount}`,
      );
    } else {
      const taskItems = page.locator('[data-testid="task-card"] [data-testid^="task-item"]');
      ...
      record(
        ...
        "8.2",
        "8.2a 展开按钮 (FR-020)",
        count >= 1 ? "PARTIAL" : "FAIL",
        `未找到展开按钮，可见任务数=${count}`,
      );
    }

    await page.screenshot({
      path: "test-results/phase0-s8-step8.2-expanded-tasks.png",
      fullPage: true,
    });
  });

  test("步骤 8.3 — 任务状态实时更新 (FR-021)", async ({ page }) => {
    const { navigated } = await createProjectAndWait(
      page,
      "任务实时更新测试",
      "分析A股走势，时长约5分钟，发在B站，面向普通投资者",
    );

    if (!navigated) {
      record(SCENARIO, "8.3", "8.3 项目创建", "FAIL", "无法创建项目");
      return;
    }
    ...
    const hasCompleted = pageText.includes("完成") || pageText.includes("成功");
    const hasRunning = pageText.includes("执行中") || pageText.includes("running");
    const hasPending = pageText.includes("待执行") || pageText.includes("pending");
    const statusCount = [hasCompleted, hasRunning, hasPending].filter(Boolean).length;

    record(
      SCENARIO,
      "8.3",
      "8.3a 任务状态更新可见 (FR-021)",
      statusCount >= 1 ? "PASS" : "FAIL",
      `完成=${hasCompleted}, 执行中=${hasRunning}, 待执行=${hasPending}`,
    );

    await page.screenshot({
      path: "test-results/phase0-s8-step8.3-task-status.png",
      fullPage: true,
    });
  });
});
...
// ═══════════════════════════════════════════════════════════════
// 场景 9: 推进按钮状态验证 (FR-023, FR-024) — T036
// ═══════════════════════════════════════════════════════════════
test.describe("场景 9: 推进按钮状态验证 (FR-023, FR-024)", () => {
  const SCENARIO = "场景9-推进按钮";

  test("步骤 9.1 — 无产物时按钮禁用", async ({ page }) => {
    const { navigated } = await createProjectAndWait(
      page,
      "无产物禁用测试",
      "分析近期黄金走势，发在B站，约5分钟",
    ...
    if (!navigated) {
      record(SCENARIO, "9.1", "9.1 项目创建", "FAIL", "无法创建项目");
      return;
    }
    ...
    if (btnVisible) {
      ...
      record(
        SCENARIO,
        "9.1",
        "9.1a 无产物时按钮禁用 (FR-024)",
        isDisabled ? "PASS" : "FAIL",
        `disabled=${isDisabled}, title="${title}"`,
      );
    } else {
      record(SCENARIO, "9.1", "9.1a 无产物时按钮禁用 (FR-024)", "FAIL", "按钮不可见");
    }

    await page.screenshot({
      path: "test-results/phase0-s9-step9.1-no-artifact-disabled.png",
      fullPage: true,
    });
  });

  test("步骤 9.2 — 产物生成完成后按钮启用并推进", async ({ page }) => {
    const { navigated } = await createProjectAndWait(
      page,
      "按钮启用测试",
      "分析黄金走势，时长约5分钟，发在B站，面向新手投资者",
    ...
    if (!navigated) {
      record(SCENARIO, "9.2", "9.2 项目创建", "FAIL", "无法创建项目");
      return;
    }
    ...
    const advanceBtn = page.locator("button", { hasText: "确认进入下一阶段" });
    await advanceBtn.waitFor({ state: "visible", timeout: 5000 }).catch(() => {});
    ...
    if (!btnEnabled) {
      ...
      record(
        SCENARIO,
        "9.2",
        "9.2a 产物完成后按钮启用 (FR-023)",
        "PARTIAL",
        `超时未启用, title="${title}"`,
      );
    } else {
      record(SCENARIO, "9.2", "9.2a 产物完成后按钮启用 (FR-023)", "PASS", "按钮已启用可推进");
      ...
      const pageText = await page.locator("body").innerText();
      const hasPhase1 =
        pageText.includes("Phase 1") || pageText.includes("阶段 1") || pageText.includes("大纲");
      record(
        SCENARIO,
        "9.2",
        "9.2b 推进后 Phase 1 激活 (FR-023)",
        hasPhase1 ? "PASS" : "PARTIAL",
        `Phase1可见=${hasPhase1}`,
      );

      await page.screenshot({
        path: "test-results/phase0-s9-step9.2-advanced-to-p1.png",
        fullPage: true,
      });
    }
  });

  test("步骤 9.3 — 审核未通过时按钮禁用", async ({ page }) => {
    await page.goto(`${BASE_URL}/projects`);
    await page.waitForLoadState("networkidle");

    const firstRow = page.locator("table tbody tr").first();
    if (!(await firstRow.isVisible().catch(() => false))) {
      record(SCENARIO, "9.3", "9.3 审核FAIL按钮禁用 (FR-024)", "SKIP", "无可用项目");
      return;
    }
    ...
    const pageText = await page.locator("body").innerText();
    const hasReviewFail =
      pageText.includes("未通过") || pageText.includes("FAIL") || pageText.includes("审核");

    record(
      SCENARIO,
      "9.3",
      "9.3a 审核未通过时按钮状态 (FR-024)",
      hasReviewFail && isDisabled ? "PASS" : "PARTIAL",
      `review_fail=${hasReviewFail}, disabled=${isDisabled}, title="${title}"`,
    );

    await page.screenshot({
      path: "test-results/phase0-s9-step9.3-review-fail-disabled.png",
      fullPage: true,
    });
  });
});
...
// ═══════════════════════════════════════════════════════════════
// 场景 10: 澄清问题快速回复 (FR-011, FR-012) — T037
// ═══════════════════════════════════════════════════════════════
test.describe("场景 10: 澄清问题快速回复 (FR-011, FR-012)", () => {
  const SCENARIO = "场景10-澄清快速回复";

  test("步骤 10.1 — 模糊描述触发澄清问题", async ({ page }) => {
    const { navigated } = await createProjectAndWait(
      page,
      "澄清快速回复测试",
      "做一个金融分析视频",
    ...
    if (!navigated) {
      record(SCENARIO, "10.1", "10.1 项目创建", "FAIL", "无法创建项目");
      return;
    }
    ...
    const hasClarification =
      pageText.includes("澄清") ||
      pageText.includes("需要补充") ||
      pageText.includes("❓") ||
      pageText.includes("⚠") ||
      pageText.includes("请确认");

    record(
      SCENARIO,
      "10.1",
      "10.1a 澄清问题区域出现 (FR-011)",
      hasClarification ? "PASS" : "PARTIAL",
      hasClarification ? "检测到澄清区域" : "无澄清区域",
    );

    const quickReplyBtns = page
      .locator("button")
      .filter({ hasText: /分钟|平台|时长|B站|抖音|新手|专业/ })
      .first();
    const hasQuickReplies = await quickReplyBtns.isVisible().catch(() => false);

    record(
      SCENARIO,
      "10.1",
      "10.1b 快速回复按钮存在 (FR-011)",
      hasQuickReplies ? "PASS" : "PARTIAL",
      hasQuickReplies ? "检测到快速回复选项按钮" : "无快速回复按钮",
    );

    await page.screenshot({
      path: "test-results/phase0-s10-step10.1-clarification.png",
      fullPage: true,
    });
  });

  test("步骤 10.2 — 点击快速回复后需求更新 (FR-012)", async ({ page }) => {
    const { navigated } = await createProjectAndWait(
      page,
      "快速回复更新测试",
      "做一个金融分析视频",
    ...
    if (!navigated) {
      record(SCENARIO, "10.2", "10.2 项目创建", "FAIL", "无法创建项目");
      return;
    }
    ...
    const btnCount = await quickReplyBtns.count().catch(() => 0);
    if (btnCount > 0) {
      const btnLabel = await quickReplyBtns.first().textContent();
      await quickReplyBtns.first().click();
      ...
      const chatInput = page.locator('[data-testid="chat-input"] textarea');
      if (await chatInput.isVisible().catch(() => false)) {
        await chatInput.fill("B站，5-8分钟，面向投资新手");
        await page.locator('[aria-label="发送消息"]').click();
        await page.waitForTimeout(4000);

        const pageText = await page.locator("body").innerText();
        const hasResponse = pageText.includes("B站") || pageText.includes("更新");
        record(
          SCENARIO,
          "10.2",
          "10.2a 聊天输入替代回复 (FR-012)",
          hasResponse ? "PARTIAL" : "FAIL",
          `需求更新=${hasResponse}`,
        );
      } else {
        record(SCENARIO, "10.2", "10.2a 快速回复", "FAIL", "无快速回复按钮且聊天输入不可用");
      }
    }

    await page.screenshot({
      path: "test-results/phase0-s10-step10.2-quick-reply.png",
      fullPage: true,
    });
  });
});
