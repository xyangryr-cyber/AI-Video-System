"""Tests for [SPEC-A-013] MasterAudioArtifact schema + ProjectState.master_audio_ref.

Authority: docs/specs/SPEC_REVISION_REQ_v3.17_2026-04-17.md A-AUDP7A-1.

Covers AC-1..AC-6 from tasks/SPEC-A/A-013-master-audio-schema.md.
"""
...
# ---------------------------------------------------------------------------
# AC-1: JSON Schema validates the three kinds (valid + invalid cases).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AC-2: Pydantic discriminated union + round-trip preserves every field.
# ---------------------------------------------------------------------------


class TestAC2PydanticRoundTrip:
    ...
    def test_narration_pydantic_rejects_source_ref(self):
        from src.shared.schemas.audio_master import MasterAudioArtifactAdapter
        ...
        with pytest.raises(PydanticError):
            MasterAudioArtifactAdapter.validate_python(payload)

    def test_bgm_pydantic_requires_source_ref(self):
        from src.shared.schemas.audio_master import MasterAudioArtifactAdapter
...
# ---------------------------------------------------------------------------
# AC-3: ProjectState.master_audio_ref is optional and typed.
# ---------------------------------------------------------------------------
...
class TestAC3ProjectStateMasterAudioRef:
...
# ---------------------------------------------------------------------------
# AC-4: artifact registry adds three master-audio entries.
# ---------------------------------------------------------------------------


class TestAC4ArtifactRegistry:
...
# ---------------------------------------------------------------------------
# AC-5: checksum chain invariants enforced in Python-level helper.
# ---------------------------------------------------------------------------


class TestAC5ChecksumChain:
...
# ---------------------------------------------------------------------------
# AC-6: cross-language field alignment across JSON Schema / Pydantic / TS.
# ---------------------------------------------------------------------------


class TestAC6CrossLanguageAlignment:
